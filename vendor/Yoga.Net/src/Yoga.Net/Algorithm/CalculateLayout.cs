...
namespace Facebook.Yoga
{
    public static partial class LayoutAlgorithm
    {
        ...
        private static readonly ConditionalWeakTable<Node, OwnerBasis> percentageOwnerBases = new();

        private sealed class OwnerBasis
        {
            public float Width;
            public float Height;
        }

        private static bool HasChangedPercentageOwnerBasis(
            Node node, float ownerWidth, float ownerHeight)
        {
            if (!node.Style.HasPercentageLength)
            {
                percentageOwnerBases.Remove(node);
                return false;
            }

            return !percentageOwnerBases.TryGetValue(node, out var basis) ||
                !Comparison.InexactEquals(basis.Width, ownerWidth) ||
                !Comparison.InexactEquals(basis.Height, ownerHeight);
        }

        private static void StorePercentageOwnerBasis(
            Node node, float ownerWidth, float ownerHeight)
        {
            if (!node.Style.HasPercentageLength) return;

            var basis = percentageOwnerBases.GetValue(node, static _ => new OwnerBasis());
            basis.Width = ownerWidth;
            basis.Height = ownerHeight;
        }
        ...
        private static float DistributeFreeSpaceSecondPass(
            ref FlexLineState flexLine, Node node,
        ...
        private static void DistributeFreeSpaceFirstPass(
            ref FlexLineState flexLine, Direction direction, FlexDirection mainAxis,
            float ownerWidth, float mainAxisOwnerSize,
        ...
        private static void ResolveFlexibleLength(
            Node node, ref FlexLineState flexLine,
            FlexDirection mainAxis, FlexDirection crossAxis, Direction direction,
            float ownerWidth, float mainAxisOwnerSize,
            float availableInnerMainDim, float availableInnerCrossDim,
            float availableInnerWidth, float availableInnerHeight,
            ...
            uint depth, uint generationCount)
        {
            ...
            float distributedFreeSpace = DistributeFreeSpaceSecondPass(
                ref flexLine, node, mainAxis, crossAxis, direction,
            ...
            ref LayoutData layoutMarkerData, uint depth, uint generationCount)
        {
            ...
            var layoutChildren = LayoutScratch.Rent((int)childCount);
            try
            {
            node.CopyLayoutChildrenTo(layoutChildren);
            ...
            totalMainDim += ComputeFlexBasisForChildren(
                ...
                performLayout, ref layoutMarkerData, depth, generationCount, layoutChildren);
            ...
            {
                var lineItems = LayoutScratch.Rent(layoutChildren.Count - startOfLineIndex);
                try
                {
                var flexLine = YogaAlgorithm.CalculateFlexLine(
                    ...
                    layoutChildren, startOfLineIndex, lineItems, lineCount);
                startOfLineIndex = flexLine.NextIndex;
                ...
                // STEP 6
                JustifyMainAxis(node, ref flexLine, mainAxis, crossAxis, direction,
                ...
                {
                    LayoutScratch.Return(lineItems);
                }
            }
            ...
            {
                LayoutScratch.Return(layoutChildren);
            }
        }
        ...
        bool needToVisitNode =
            ...
            HasChangedPercentageOwnerBasis(node, ownerWidth, ownerHeight);
        ...
        else
        {
            ...
            layout.LastOwnerDirection = ownerDirection;
                layout.ConfigVersion = node.Config.GetVersion();
                StorePercentageOwnerBasis(node, ownerWidth, ownerHeight);
