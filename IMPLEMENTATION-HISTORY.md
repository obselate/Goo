...
- S16-D03 keeps direct Wayland presentation feedback deferred and accepts nominal display refresh until a supported path is available or cadence fails. It is not an active local blocker.
...
- Froze source commit `6d4d92e8b0046d51cc5520caa3fccd048d367332` and ran all eight official Linux workloads with one 5,815,728-byte NativeAOT binary. All 40 processes pass exact submit/present, both-slot, absolute-frame, warm-resource, diagnostics, cleanup, and hash contracts. Five isolated true-idle processes pass at median `0.0997%` of one core with zero work or allocation.
- Clean-source validation passes API contracts 10/10, core behavior 262/262, 2,000 Effects/Offscreen stage samples with exact 16/8 scopes and zero drops, seven package Wayland lanes, release validation, default and native-window package smoke, and NativeAOT package-consumer execution. The package is `3,783,856` bytes, validated bundle `10,087,660` bytes, and package-consumer NativeAOT executable `5,499,784` bytes.
- Local discrete-Linux implementation and qualification are complete. Remaining work is external hardware. Evidence is `artifacts/reports/s15-q10/clean-linux-6d4d92e-summary.json` plus its SHA-256 manifest.
- Dropped every active Vulkan-versus-Skia comparison by decision. Skia and reconstructed-control measurements remain archive context only. Clean commit `6d4d92e` Vulkan workload, startup, memory, package, bundle, and NativeAOT values are the regression references; independent absolute correctness, frame, latency, resource, distribution, fallback, and validation gates remain.
