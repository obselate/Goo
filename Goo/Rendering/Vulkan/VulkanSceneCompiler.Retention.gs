...
internal partial class VulkanSceneCompiler {
    ...
    private func RetainedBorderColorsFinite(node Node) bool {
        return Finite(node.BorderTopColor.R) && Finite(node.BorderTopColor.G)
    ...
    private func RetainedBorderCandidate(node Node, bounds ConservativeBounds) bool {
    ...
    private func RetainedLeafEligible(
        ...
        parentIsolated bool,
        activeClipBounds ConservativeBounds) bool {
        return RetainedBoxEligible(node, bounds, opacity, parentTransformIndex,
            parentClipIndex, parentOpacity, parentAxisAligned, parentClipDepth,
            parentPathClipChainId, parentIsolated, activeClipBounds, true)
    }
    ...
    private func TryAppendRetainedLeaf(
        ...
        ownerId uint64,
        bounds ConservativeBounds,
        opacity float32) bool {
        return TryAppendRetainedBox(node, owner, ownerId, bounds, opacity, true)
    }
    ...
    private func TryAppendRetainedBorderLeaf(
        ...
        bounds ConservativeBounds,
        record PerEdgeBorderRecord) bool {
        return TryAppendRetainedBorder(node, owner, ownerId, bounds, record)
    }
    ...
    private func ClassifyRetainedChunks() {
        ...
        if !proofs.Ready {
            ...
            while index < maximum {
                ...
                let oldBounds = hasOld ? retainedChunks[index].Bounds
                    : ConservativeBounds{}
    ...
    private func ExactSolid(left SolidBoxRecord, right SolidBoxRecord) bool {
        return ExactBounds(left.Bounds, right.Bounds)
            && left.Color == right.Color
            && ExactFloat(left.Opacity, right.Opacity)
            && left.TransformIndex == right.TransformIndex
    }

    private func ExactRounded(left RoundedBoxRecord, right RoundedBoxRecord) bool {
        return ExactBounds(left.Bounds, right.Bounds)
    ...
    private func ExactBorder(left PerEdgeBorderRecord, right PerEdgeBorderRecord) bool {
        return ExactBounds(left.Bounds, right.Bounds)
    ...
    private func ExactCachedTextSegment(
        ...
        right CachedTextSegmentRefRecord) bool {
        return ExactBounds(left.Bounds, right.Bounds)
    ...
    private func ExactResource(left ResourceId, right ResourceId) bool {
