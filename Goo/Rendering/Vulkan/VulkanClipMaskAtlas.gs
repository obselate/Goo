...
private unsafe sealed class VulkanClipMaskAtlasGeneration : IDisposable {
    ...
    private func FormatValue() VkFormat {
        return if Format == VulkanClipMaskFormat.R8Unorm {
...
internal unsafe sealed partial class VulkanClipMaskAtlas : IDisposable {
    ...
    internal func Acquire(key VulkanClipMaskAtlasKey, screenX int32, screenY int32,
        ...
        return Acquire(key.Value, screenX, screenY, screenWidth, screenHeight)
    }
    ...
    private func CurrentUsageBatchToken() uint64 {
        return if usageBatchOpen { usageBatchToken } else { protectedUsageBatchToken }
    }
