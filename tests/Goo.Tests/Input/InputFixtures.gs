...
internal class InputFixtures {
  ...
  func UnhandledWheelStaysPresentationIdle() bool {
    ...
    return !changed && !window.UpdateTreeForPresentation(0.0)
  }
  ...
  func Sdl3KeyAdapterMapsAllKeysAndModifiers() bool {
    for i in 0 ... int32(SdlHostKey.Menu) + 1 {
      let hostKey = SdlHostKey(i)
      let expected = i <= int32(SdlHostKey.F24) ? i : i + 1
      if int32(fromSdlKey(hostKey)) != expected {
        return false
      }
    }
    let modifiers = fromSdlModifiers(SdlHostModifiers(true, true, true, true))
    if !modifiers.Alt || !modifiers.Shift || !modifiers.Ctrl || !modifiers.Super {
      return false
    }
    return fromSdlPointerButton(SdlHostPointerButton.None) == PointerButton.None
      && fromSdlPointerButton(SdlHostPointerButton.Primary) == PointerButton.Primary
      && fromSdlPointerButton(SdlHostPointerButton.Secondary) == PointerButton.Secondary
      && fromSdlPointerButton(SdlHostPointerButton.Middle) == PointerButton.Middle
      && fromSdlPointerButton(SdlHostPointerButton.Back) == PointerButton.Back
      && fromSdlPointerButton(SdlHostPointerButton.Forward) == PointerButton.Forward
      && fromSdlPointerButtons(SdlHostPointerButtons.Primary | SdlHostPointerButtons.Back)
        == (PointerButtons.Primary | PointerButtons.Back)
      && fromSdlPointerButtons(SdlHostPointerButtons.Secondary | SdlHostPointerButtons.Middle
        | SdlHostPointerButtons.Forward)
        == (PointerButtons.Secondary | PointerButtons.Middle | PointerButtons.Forward)
  }
  ...
  func RoutedKeyCallbackRebuildStaysPresentationIdle() bool {
    ...
    return !pressed && !repeated && cell.Calls == 2 && !window.UpdateTreeForPresentation(0.5)
  }
  ...
  func GenericTextCallbackRebuildStaysPresentationIdle() bool {
    ...
    input.QueueText("x")
    let changed = input.Drain(window.Tree, resolver, 0.0, nil)
    return !changed && cell.Calls == 1 && !window.UpdateTreeForPresentation(0.0)
  }
