...
func RunS15TextTransportGate() {
  ...
  try {
    let opened = Window{
      Title: "Goo S15 text transport gate",
      Width: 96,
      Height: 64,
    ...
    S14Require(sawInitial,
      "S15 initial text transport did not force a complete upload")
    S14Require(initialText.ByteCount > 0uL,
      "S15 initial text transport has no text bytes")
    S14Require(initialClip.MaskCount == 0
        && initialClip.ByteCount == 16uL,
      "S15 text transport clip payload is not the no-mask 16-byte header")
    S14Require(initialPrimitive.RecordCount < initialText.RecordCount,
      "S15 analytic capacity included text records")
    S14Require(initialScene.RetainedTextRebuildCount >= 2uL,
      "S15 initial retained text did not capture both text nodes")
    ...
    S14Require(sawWarm,
      "S15 unchanged text transport did not produce a warmed slot")
    S14Require(warmText.SegmentCount == initialText.SegmentCount
    ...
    S14Require(sawPartial,
      "S15 mutated text transport did not produce a warmed partial update")
    S14Require(partialText.SegmentCount == initialText.SegmentCount
    ...
    opened.RequestClose()
    WindowReadbackTestFixture.ForceRender(opened, 0.0)
    S14Require(!opened.IsOpen, "S15 text transport gate window did not close")
  } finally {
