...
func RunS16VSyncTransitionGate() {
  ...
  try {
    let opened = Window{
      Title: "Goo S16 VSync transition gate",
      Width: 320,
      Height: 180,
      VSync: true,
      Root: root,
    ...
    S14Require(initialGeneration > 0uL
        && initialMode == VkConstants.VK_PRESENT_MODE_FIFO_KHR,
      "S16 VSync-on did not create a FIFO swapchain")
    ...
    S14Require(!opened.IsOpen, "S16 VSync transition gate window did not close")
